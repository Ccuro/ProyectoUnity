...
public class Stage1_1_Event : MonoBehaviour
{
    ...
    IEnumerator Cut1()
    {
        ...
        for (int i = 0; i < rosaryBullet.Length; i++)
        {
            ...
            rosaryBullet[i].GetComponent<Rigidbody2D>().linearVelocity = (Vector2.left + Vector2.down) * 5f;
        ...
        {
            time -= Time.deltaTime;
            Player.instance.GetComponent<Rigidbody2D>().linearVelocity = Vector2.left * 0.3f;
            yield return null;
        }
        ...
        {
            gaiten.GetComponent<Rigidbody2D>().linearVelocity = (Vector2.left * 9f);
            yield return null;
        }
    ...
    IEnumerator Cut2()
    {
        ...
        {
            time -= Time.deltaTime;
            Player.instance.GetComponent<Rigidbody2D>().linearVelocity = Vector2.right * 3f;
            yield return null;
        }
        ...
        while (delay > 0)
        {
            ...
            gaiten.GetComponent<Rigidbody2D>().linearVelocity = Vector2.left;
            yield return null;
        }
