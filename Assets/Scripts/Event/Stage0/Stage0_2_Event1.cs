...
public class Stage0_2_Event1 : MonoBehaviour
{
    ...
    {
        if (onRun)
            Player.instance.GetComponent<Rigidbody2D>().linearVelocity = new Vector2(1.3f, 0);
    }
    ...
    void MeteorSpawn()
    {
        for (int i = 0; i < meteors.Length; i++)
        {
            ...
            meteors[i].GetComponent<Rigidbody2D>().linearVelocity = new Vector2(1, -1);
        }
    }
    ...
    IEnumerator Cut1()
    {
        ...
        flyingDemon.GetComponent<Rigidbody2D>().linearVelocity = new Vector2(1.1f, 0);

        yield return new WaitForSeconds(3f);
        flyingDemon.GetComponent<Animator>().Play("Meteor");
        flyingDemon.GetComponent<Rigidbody2D>().linearVelocity = Vector2.zero;
